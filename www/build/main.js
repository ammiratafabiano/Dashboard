        ...
        currentTab.details.detailsOptions.forEach(function (element) {
            if (element.filter == 1) {
                if (element.paramName != element.name) {
                    values.push(element.elements.filter(function (x) { return x.name == element.paramName; })[0].id);
                }
                else {
                    error = 1;
                }
